...
class BetterIndex(object):
    ...
    def wildcard_search_or(self, text):
        ...
        # tokenize text
        termData = self.tokenize(text, True)
        ...
        for term in termData:
            ...
            # crawl the tree from root
            fileIndexs = crawl_tree(self._bt.root, term)
        ...
        fileIndexList = sorted(set(fileIndexList))
        
        # find file name for return indexs
        result = self.getFileNameList(fileIndexList)
    ...
    def wildcard_search_and(self, text):
        ...
        for term in termData:
            ...
            # add to result pool
            fileIndexList.append(fileIndexs)

        # get intersection of lists and sort
        fileIndexList = sorted(set.intersection(*map(set, fileIndexList)))
    ...
    def getFileNameList(self, fileIndexList):
        result = []
        for fileIndex in fileIndexList:
            filename = self._documents[fileIndex]
